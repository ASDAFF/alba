$(document).ready(function(){
    ...
    if(container.length){
    	(function(){
        	var banners = container.find('.collection_banner').get(),
        		textcontainer = $('.text_container').get(),
        		item = $('.parallax_container.type1').children('ul').children('li'),
        		item2 = $('.parallax_container.type2').children('ul').children('li'),
        		len = item.length,
        		len2 = item2.length,
        		htmlstring = "";
        	
        	htmlstring += banners[0].outerHTML + '<div class="collection_m_wrap">';
        	for(var i = 0; i < len; i++){
        		var imgsrc = item.eq(i).css('background-image').slice(4,-1);
        		console.log(imgsrc);
        			// htmlitem = item.eq(i).html();
        		htmlstring += '<figure class="mobile_home_item"><img src=' + imgsrc + ' alt=""></figure>';
        	}
        	htmlstring += '</div>' + textcontainer[0].outerHTML;
        	htmlstring += banners[1].outerHTML + '<div class="collection_m_wrap">';
        	for(var i = 0; i < len2; i++){
        		var imgsrc = item2.eq(i).css('background-image').slice(4,-1);
        			// htmlitem = item2.eq(i).html();
        		htmlstring += '<figure class="mobile_home_item"><img src=' + imgsrc + ' alt=""></figure>';
        	}
        	htmlstring += "</div>";
        	$('#mobile_copy_home').append(htmlstring);
        })();
    }
	...
	isLowWidth();
	window.onresize = isLowWidth;



    /*lookbook*/
    if($('.lookbook_list').length){
        var lookbook = $(".lookbook_list").owlCarousel({
            items: 2,
            itemsDesktop:	[1199,2],
        ...
        (function(){
        	var item = lookbook.find('.owl-item'),
        		len = item.length,
        		htmlstring = "";
        	for(var i = 0; i < len; i++){

        		var imgsrc = item.eq(i).find('.normal-image').css('background-image').slice(4,-1),
        			htmlitem = "";
        			if(item.eq(i).find('.shop_mini_section').length){
        			 htmlitem += item.eq(i).find('.shop_mini_section').html();
        			}
        			
        		htmlstring += '<figure class="mobile_lookbook_item"><img src=' + imgsrc + ' alt=""><ul class="shop_mini_section t_align_left clearfix">' + htmlitem + '</ul></figure>';
        	}
        	$('#m_lookbook').append(htmlstring);
        })();
    }
    if($('.lookbook').length){
...
$(document).ready(function () {
	$('a.select_year').on("click", function () {
		$('ul.histor_list').find('li').removeClass('current').removeClass('or');
		$(this).parent().addClass('current').addClass('or');
		$('div.history_block').hide().parent();
		$('div.history_block.' + $(this).data('year')).fadeIn();
	});
    ...
    $('a.prod_tabs').on('click', function () {
        $('div.prod_tab_container').hide();
        $('.prod_tabs_menu').find('li').removeClass('active');
        $('div.prod_tab_container.' + $(this).data('container')).fadeIn();
        $(this).parent().addClass('active');
    });

    if ($('div.success_message_partner') !== undefined) {
        $('div.success_message_partner_show').html($('div.success_message_partner').html()).show();
    }
